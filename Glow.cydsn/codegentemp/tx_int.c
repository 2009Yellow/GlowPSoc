...
void tx_int_Start(void)
{
    ...
    /* Set the priority. */
    tx_int_SetPriority(tx_int_INTC_PRIOR_NUMBER);
...
*   the component has been started use tx_int_StartEx instead.
*
*
* Parameters:
*   address: Address of the ISR to set in the interrupt vector table.
...
void tx_int_SetVector(cyisraddress address) 
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + tx_int__INTC_NUMBER] = address;
}
...
cyisraddress tx_int_GetVector(void) 
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + tx_int__INTC_NUMBER];
}
...
*   or tx_int_StartEx will override any effect this method would have had. 
*	This method should only be called after tx_int_Start or 
*	tx_int_StartEx has been called. To set the initial
...
uint8 tx_int_GetState(void) 
{
    ...
    return (*tx_int_INTC_SET_EN & tx_int__INTC_MASK) ? 1:0;
}
